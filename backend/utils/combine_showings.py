...
import os
import requests
import title_normalizer  # Import title_normalizer to access shared config
...
# Output file (to site's data folder via mounted volume)
output_dir = Path("/app/frontend_data")
output_path = output_dir / "movies.json"
output_dir.mkdir(parents=True, exist_ok=True)

# Load blacklist showings from shared config
BLACKLIST_SHOWINGS = set(title_normalizer.load_shared_config().get('blacklistShowings', []))
...
def main():
    ...
    # Trigger automatic poster pre-fetching (skip if running from backend pipeline to avoid circular call)
    skip_prefetch = os.getenv("SKIP_POSTER_PREFETCH", "false").lower() == "true"

    if not skip_prefetch:
        print(f"\n{Colors.BLUE}Triggering poster pre-fetch...{Styles.default}")
        try:
            backend_url = os.getenv("BACKEND_URL", "http://localhost:3001")
            response = requests.post(
                f"{backend_url}/api/prefetch-posters",
                timeout=320  # 5min 20s (backend has longer timeout for many posters)
            )

            if response.status_code == 200:
                result = response.json()
                print(f"{Colors.GREEN}✓ Posters: {result.get('fetched', 0)} new, "
                      f"{result.get('cached', 0)} cached, "
                      f"{result.get('failed', 0)} failed{Styles.default}")
                if result.get('failed', 0) > 0:
                    failed_titles = result.get('failed_titles', [])
                    if failed_titles:
                        print(f"{Colors.YELLOW}Failed titles: {', '.join(failed_titles[:5])}{'...' if len(failed_titles) > 5 else ''}{Styles.default}")
            else:
                print(f"{Colors.YELLOW}⚠ Poster pre-fetch returned status {response.status_code}{Styles.default}")
        except requests.exceptions.Timeout:
            print(f"{Colors.YELLOW}⚠ Poster pre-fetch timed out (may still be processing in background){Styles.default}")
        except Exception as e:
            print(f"{Colors.YELLOW}⚠ Poster pre-fetch failed (non-critical): {e}{Styles.default}")
    else:
        print(f"\n{Colors.YELLOW}Skipping poster pre-fetch (running from backend pipeline){Styles.default}")
