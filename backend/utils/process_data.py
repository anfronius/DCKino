...
# Clean old data files, keeping only the newest raw and processed file per theater
def clean_old_data_files():
    print(f"\n{Colors.BLUE}Cleaning old data files... {Styles.default}\n")
    for theater in theaters:
        theater_dir = data_dir / theater
        if not theater_dir.exists():
            continue

        # Clean raw files - keep only the newest
        raw_files = sorted(theater_dir.glob("*_raw_*.json"), key=lambda f: f.stat().st_mtime, reverse=True)
        if len(raw_files) > 1:
            kept_file = raw_files[0].name
            for old_file in raw_files[1:]:  # Keep first (newest), delete rest
                print(f"{Colors.YELLOW}Deleting old raw: {old_file.name}{Styles.default}")
                old_file.unlink()
            print(f"{Colors.GREEN}Kept newest raw for {theater}: {kept_file}{Styles.default}")
        elif len(raw_files) == 1:
            print(f"{Colors.GREEN}Only one raw file for {theater}: {raw_files[0].name}{Styles.default}")

        # Clean processed files - keep only the newest
        processed_files = sorted(theater_dir.glob("*_processed_*.json"), key=lambda f: f.stat().st_mtime, reverse=True)
        if len(processed_files) > 1:
            kept_file = processed_files[0].name
            for old_file in processed_files[1:]:  # Keep first (newest), delete rest
                print(f"{Colors.YELLOW}Deleting old processed: {old_file.name}{Styles.default}")
                old_file.unlink()
            print(f"{Colors.GREEN}Kept newest processed for {theater}: {kept_file}{Styles.default}")
        elif len(processed_files) == 1:
            print(f"{Colors.GREEN}Only one processed file for {theater}: {processed_files[0].name}{Styles.default}")

    print(f"\n{Colors.GREEN}Data cleanup complete. Only newest files retained per theater.{Styles.default}")
